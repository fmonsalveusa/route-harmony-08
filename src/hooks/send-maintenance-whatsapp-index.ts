...
function cleanPhone(phone: string): string | null {
  ...
  // Si tiene 10 dígitos, agrega +1 (USA)
  if (digits.length === 10) return `+1${digits}`;
  // Si tiene 11 y empieza en 1, agrega +
...
serve(async (req) => {
  ...
  try {
    ...
    const { maintenanceId, maintenanceType, status, milesAccumulated, truckId } = await req.json();

    if (!truckId || !maintenanceType || !status) {
      return new Response(JSON.stringify({ error: "Missing required fields" }), { status: 400, headers: corsHeaders });
    }

    // Buscar el driver asignado al camión
    ...
    const statusLabel = status === "due" ? "VENCIDO" : "Próximo a Vencer";
    const estadoLabel = status === "due" ? "Vencido" : "Próximo a vencer";
