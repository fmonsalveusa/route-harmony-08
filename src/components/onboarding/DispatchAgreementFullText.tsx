...
import type { DispatchCompanyData } from './DispatchServiceOnboarding';
...
// Render bilingue del Dispatch Service Agreement de 58 Logistics LLC.
// Los datos del cliente se sustituyen en tiempo real.
// Este mismo texto sera la base para el PDF generado en Fase 5.
export function DispatchAgreementFullText({ company, signerName }: Props) {
  ...
  const Clause = ({ number, titleEn, titleEs, en, es }: { number: string; titleEn: string; titleEs: string; en: string; es: string }) => (
    <div className="space-y-2 py-2">
      <p className="text-sm font-bold text-primary">{number}. {titleEn} / {titleEs}</p>
      <p className="text-xs leading-relaxed">{en}</p>
      <p className="text-xs leading-relaxed italic text-muted-foreground">{es}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="text-center border-b pb-3">
        <p className="text-sm font-bold">58 LOGISTICS LLC</p>
        <p className="text-xs text-muted-foreground">DISPATCH SERVICE AGREEMENT / CONTRATO DE SERVICIOS DE DISPATCH</p>
        <p className="text-[10px] text-muted-foreground mt-1">Bilingual English–Spanish Agreement</p>
        <p className="text-xs mt-2"><strong>Effective Date / Fecha de Vigencia:</strong> {today}</p>
      </div>

      {/* Client info table */}
      <div className="border rounded p-3 bg-muted/20">
        <p className="text-xs font-semibold uppercase text-muted-foreground mb-2">Client / Cliente</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-xs">
          <div><span className="text-muted-foreground">Legal Business Name:</span> <strong>{legal}</strong></div>
          {dba && <div><span className="text-muted-foreground">DBA:</span> <strong>{dba}</strong></div>}
          <div><span className="text-muted-foreground">MC #:</span> <strong>{mc}</strong></div>
          <div><span className="text-muted-foreground">USDOT #:</span> <strong>{dot}</strong></div>
          <div className="md:col-span-2"><span className="text-muted-foreground">Address:</span> <strong>{address}</strong></div>
          <div><span className="text-muted-foreground">Phone:</span> <strong>{phone}</strong></div>
          <div><span className="text-muted-foreground">Email:</span> <strong>{email}</strong></div>
          <div className="md:col-span-2"><span className="text-muted-foreground">Owner/Authorized Representative:</span> <strong>{owner}</strong></div>
        </div>
      </div>
      ...
      <Clause
        ...
        titleEn="PARTIES"
        titleEs="PARTES"
        en={`This Dispatch Service Agreement ("Agreement") is entered into between 58 Logistics LLC, 11634 N Wind Place Apt 206, Charlotte, NC 28210, Phone: (704) 727-6015, Email: 58logisticsllc@gmail.com ("Dispatcher"), and ${legal} ("Client" or "Carrier").`}
        es={`El presente Contrato de Servicios de Dispatch ("Contrato") se celebra entre 58 Logistics LLC, 11634 N Wind Place Apt 206, Charlotte, NC 28210, Teléfono: (704) 727-6015, Email: 58logisticsllc@gmail.com ("Dispatcher"), y ${legal} ("Cliente" o "Carrier").`}
      />

      <Clause
        number="2"
        titleEn="PURPOSE AND NATURE OF SERVICES"
        titleEs="PROPÓSITO Y NATURALEZA DE LOS SERVICIOS"
      ...
      <Clause
        number="4"
        titleEn="CLIENT'S FINAL DECISION"
        titleEs="DECISIÓN FINAL DEL CLIENTE"
      ...
      <Clause
        number="5"
        titleEn="CLIENT-FIRST REPRESENTATION"
        titleEs="REPRESENTACIÓN EN EL MEJOR INTERÉS DEL CLIENTE"
      ...
      <Clause
        number="6"
        titleEn="DISPATCH FEE AND WEEKLY BILLING"
        titleEs="COMISIÓN DE DISPATCH Y FACTURACIÓN SEMANAL"
      ...
      <Clause
        number="7"
        titleEn="FACTORING ASSISTANCE"
        titleEs="ASISTENCIA CON FACTORING"
      ...
      <Clause
        number="9"
        titleEn="LOADS BOOKED BEFORE TERMINATION"
        titleEs="CARGAS RESERVADAS ANTES DE LA TERMINACIÓN"
      ...
      <Clause
        number="10"
        titleEn="NO GUARANTEE OF LOADS OR REVENUE"
        titleEs="NO GARANTÍA DE CARGAS O INGRESOS"
      ...
      <Clause
        number="11"
        titleEn="INDEPENDENT CONTRACTOR; OPERATIONAL CONTROL"
        titleEs="CONTRATISTA INDEPENDIENTE; CONTROL OPERACIONAL"
      ...
      <Clause
        number="12"
        titleEn="LIABILITY AND CLAIMS"
        titleEs="RESPONSABILIDAD Y RECLAMACIONES"
      ...
      <Clause
        number="13"
        titleEn="DOCUMENTATION AND CLIENT COOPERATION"
        titleEs="DOCUMENTACIÓN Y COOPERACIÓN DEL CLIENTE"
      ...
      <Clause
        ...
        titleEs="VIGENCIA Y TERMINACIÓN"
      ...
      <Clause
        number="16"
        titleEn="GOVERNING LAW AND VENUE"
      ...
      <Clause
        number="17"
        titleEn="ENTIRE AGREEMENT; AMENDMENTS"
        titleEs="ACUERDO COMPLETO; MODIFICACIONES"
      ...
      <Clause
        number="18"
        titleEn="SEVERABILITY & LANGUAGE"
        titleEs="DIVISIBILIDAD E IDIOMA"
      ...
      <div className="border-t pt-3 mt-3 text-[10px] text-muted-foreground italic">
        <strong>IMPORTANT:</strong> This document is a business contract template and is not a substitute for advice from a licensed attorney. /
        Este documento es una plantilla de contrato comercial y no sustituye el asesoramiento de un abogado con licencia.
      </div>
    </div>
  );
}
