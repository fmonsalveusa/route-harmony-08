...
import { X, Plus, Upload, RotateCcw, Contrast, Trash2, ScanLine } from 'lucide-react';
...
function enhanceImage(dataUrl: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Grayscale + adaptive threshold for document look
...
export const DocumentScanner = ({ open, onClose, stop, loadRef, driverName, onUpdate }: DocumentScannerProps) => {
  ...
  const currentSrc = currentPage
    ? (currentPage.showEnhanced && currentPage.enhanced ? currentPage.enhanced : currentPage.original)
    : null;
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col">
      ...
      <div className="flex flex-wrap gap-2 px-4 py-3 bg-black/90 justify-center">
        <Button variant="outline" size="sm" onClick={triggerCapture} className="gap-1.5 text-xs bg-white/10 border-white/20 text-white hover:bg-white/20">
          {pages.length === 0 ? <><ScanLine className="h-4 w-4" /> Escanear</> : <><Plus className="h-4 w-4" /> Agregar pág.</>}
        </Button>
