...
export function generateServiceAgreementPdf(data: {
  ...
  const spClauses: [string, string][] = [
    ...
    ['QUINTA. INSPECCIONES EN CARRETERA', 'En caso de que EL DRIVER sea sometido a una inspección en carretera (DOT o similar), debe ser notificado de manera inmediata, para tal inspección EL DRIVER utilizará siempre la información correspondiente a su propia empresa la cual debe estar colocada a cada lado de su vehículo. Nunca debe usar la información de nuestra empresa.\n\nEn caso que en el reporte de la inspección aparezca el nombre de nuestra empresa, el pago correspondiente a la carga que lleve en ese momento será retenido.'],
  ...
  const enClauses: [string, string][] = [
    ...
    ['FIFTH. ROADSIDE INSPECTIONS', 'In the event THE DRIVER is subject to a roadside inspection (DOT or similar), THE DRIVER must notify THE COMPANY immediately. For such inspection, THE DRIVER must always use the information of his/her own company, which must be displayed on both sides of the vehicle. THE COMPANY\'s information must never be used. If the inspection report shows the name of THE COMPANY, the corresponding payment for the load being carried at that time will be withheld.'],
